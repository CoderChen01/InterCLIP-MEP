...
class RobertaSelfAttention(nn.Module):
    def __init__(
        self, config, position_embedding_type=None, is_add_fusion: bool = False
    ):
        super().__init__()
    ...
    def forward(
        ...
        if conditional_hidden_states is not None and self.is_add_fusion:
            context_layer = context_layer[:, :original_tag_len, :]
...
class RobertaSelfOutput(nn.Module):
    ...
    def forward(
        ...
        if self.is_add_fusion:
            hidden_states = self.dense(hidden_states) + self.gated_proj(
                hidden_states
            ) * torch.nn.functional.tanh(self.gating_param)
        else:
...
class RobertaAttention(nn.Module):
    def __init__(
        ...
        self.self = RobertaSelfAttention(
            ...
            is_add_fusion=is_add_fusion,
        )
        self.output = RobertaSelfOutput(config, is_add_fusion)
...
class RobertaLayer(nn.Module):
    def __init__(self, config, is_add_fusion: bool = False):
        ...
        self.seq_len_dim = 1
        self.attention = RobertaAttention(config, is_add_fusion=is_add_fusion)
        self.is_decoder = config.is_decoder
...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            concat_attention_mask = torch.nn.functional.pad(
                attention_mask, (0, n), "constant", value=0
            )
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            args = (
                ...
                if self.is_conditional and i in self.cond_attn_layer_inds
                else (
                    hidden_states,
                    attention_mask,
                    layer_head_mask,
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    layer_module.__call__, *args
                )
            else:
                layer_outputs = layer_module(*args)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = (
            self.pooler(sequence_output, (input_ids != self.config.pad_token_id).long())
